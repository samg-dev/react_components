...
// a picture list. This pictures are fetched from Instagram via AJAX

var Picture = React.createClass({

    // This component doesn't hold any state - it simple transforms
...
var PictureList = React.createClass({
	...
	getInitialState: function(){
	    ...
	    // the favorites one when the user clicks an image:
	
            return { picures: [], favorites: [] };
	    ...
	    var url = 'http://api.instagram.com/v1/media/popular?client_id=' + this.props.apiKey + 
'&callback=?';
	...
	// Remove the picture from the favorites array
	favorites.splice(i, 1);

	for(i = 0; i < pictures.length; i++){
	    if(pictures[i].id == id) {
		pictures[i].favorites = false;
		break;
	    }
	}
...
// I am using an API key for an Instagram test ap. Please generate and
